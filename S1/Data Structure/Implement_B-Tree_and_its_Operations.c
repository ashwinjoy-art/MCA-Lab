...
void splitChild(struct BTreeNode *x, int i, struct BTreeNode *y) 
{
    ...
    // If y is not a leaf, copy the children as well
    if (!y->isLeaf) {
    ...
    // Shift the children of x to make space for the new child z
    ...
    // Shift the keys of x to make space for the key from y
    for (int j = x->numKeys - 1; j >= i; j--)
        x->keys[j + 1] = x->keys[j];

    // Insert the middle key from y to x
